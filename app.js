...
function renderGameTree(nodes, edges, sampledPath) {
    ...
    const PCOLS = ['#58a6ff', '#bc8cff'];
    const ECOLS = ['#f85149', '#3fb950', '#58a6ff'];
    ...
    function assignY(id, depth) {
        ...
        if (!kids || kids.length === 0) {
            posMap[id] = { x: depth * hGap + nodeR + 8, y: leafSlot * vSlot + nodeR + 8 };
            leafSlot++;
            return;
        }
    ...
    edges.forEach(e => {
      ...
      opacity="${opacity}" marker-end="${marker}"/>`;
        ...
        if (pct >= 5 || onPath) {
            ...
            const lOp = onPath ? 1.0 : Math.min(0.65, opacity + 0.15);
            html += `<text x="${lx}" y="${ly}" text-anchor="middle"
        font-size="9" fill="${col}" opacity="${lOp}">${label}</text>`;
        }
    });
